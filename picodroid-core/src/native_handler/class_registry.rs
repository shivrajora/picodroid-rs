...
mod tests {
    ...
    /// Every owner in `kotlin-shim/jdk-allowlist.tsv` — the `java/**` surface
    /// the Kotlin shim and Kotlin apps reference, enforced tuple-by-tuple by
    /// `:kotlin-shim:contractCheck` — must be a class pico-jvm serves: a
    /// builtin, a `BUILTIN_SUPER`/`BUILTIN_INTERFACES` name, or an SDK
    /// `java/**` class. The Gradle side proves "every reference is
    /// allowlisted"; this side proves "every allowlisted class exists", so
    /// the two cannot drift apart silently.
    #[test]
    fn jdk_allowlist_owners_are_served() {
        const ALLOWLIST: &str = include_str!("../../../kotlin-shim/jdk-allowlist.tsv");
        /// Names the JVM tolerates without a dispatcher or table row, each
        /// for one specific generic mechanism.
        const NAME_ONLY: &[(&str, &str)] = &[
            (
                "java/util/Locale",
                "getstatic on a classfile-less class reads the unset static as null (Kotlin uppercase()/lowercase())",
            ),
            (
                "java/util/AbstractList",
                "compile-time superclass of the shim's EnumEntriesList; its <init> falls through to Object",
            ),
            (
        ...
        let sdk: Vec<&str> = crate::framework_classes::FRAMEWORK_CLASSES
            .iter()
            .map(|bytes| {
                let cf = ClassFile::parse(bytes).expect("parse framework class");
                let loaded = core::str::from_utf8(cf.class_name().expect("class name"))
                    .expect("class name is UTF-8");
                crate::shrink_names::unshrink_class(loaded)
            })
            .collect();
        let served = |owner: &str| -> bool {
            BUILTIN_CLASS_NAMES.contains(&owner)
                || pico_jvm::interpreter::BUILTIN_SUPER
                    .iter()
                    .any(|(k, v)| *k == owner || *v == owner)
                || pico_jvm::interpreter::BUILTIN_INTERFACES
                    .iter()
                    .any(|(k, vs)| *k == owner || vs.contains(&owner))
                || sdk.contains(&owner)
                || NAME_ONLY.iter().any(|(n, _)| *n == owner)
        };
        ...
        for line in ALLOWLIST.lines() {
            ...
            let owner = line.split('\t').next().unwrap_or("");
            assert!(
                owner.starts_with("java/"),
                "allowlist row does not name a java/** owner: {line:?}"
            );
            if !owners.contains(&owner) {
                owners.push(owner);
            }
        }
        let missing: Vec<&str> = owners.iter().copied().filter(|o| !served(o)).collect();
        assert!(
            missing.is_empty(),
            "{} allowlisted java/** class(es) are not served by pico-jvm (not in \
             BUILTIN_CLASS_NAMES, BUILTIN_SUPER/BUILTIN_INTERFACES, the SDK, or \
             NAME_ONLY) — a Kotlin app referencing them dies at runtime: {missing:?}",
            missing.len()
        );
        assert!(
            owners.len() > 20,
            "allowlist parsed to only {} owners — include_str! path or format broke",
            owners.len()
        );
    }
}
