...
//!
//! Member names take the other route: there is no runtime translation.
//! [`m`] holds one `const` per SDK method name whose *value* is whatever
//! the loaded framework spells it (`m::setText` is `"setText"` in a
//! no-shrink build and the map's target under `--shrink`), so a `match`
//! arm written `m::setText =>` compiles to the same code as the literal it
//! replaced. Generated from `sdk/member-names.tsv` + the active map.

// Defines: pub fn unshrink_class(name: &str) -> &str;
include!(concat!(env!("OUT_DIR"), "/framework_unshrink.rs"));
// Defines: pub mod m; #[cfg(test)] unshrink_member / shrink_member.
include!(concat!(env!("OUT_DIR"), "/framework_member_names.rs"));
