...
/**
 * Aggregates the standalone term-style demos into one PAPK so HIL can verify them with a single
 * build+flash cycle. Each sub-demo is invoked via its public {@code static run()}; exceptions are
 * caught per-demo so a single failure doesn't suppress the patterns of every demo that follows it.
 */
