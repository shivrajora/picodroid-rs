...
/**
 * The kotlin-shim contract (roadmap Session 5), two directions:
 *
 ...
 *   superinterface, under its `@ShimName`). A member the walk finds only on a
 *   `java/…` supertype is served when the generated API contract
 *   ([ApiContract], `sdk/api-contract.tsv`) serves it there. Owners on the
 *   never-shimmed list are rejected with a docs pointer. The report prints
 *   ready-to-paste Java signatures grouped by class.
 ...
 *
 * The former Direction C (a hand-maintained `java/…` allowlist) is subsumed
 * by `verifyApiContract`, which every app — the fixtures included, with their
 * staged shim classes — runs against the generated contract.
 *
...
object ShimContract {
    ...
    data class ContractReport(
        ...
        val ok: Boolean
            get() = missing.isEmpty() && rejected.isEmpty() && (!strictUnused || unused.isEmpty())
    ...
    fun check(
        ...
        // Shim classes below each type: the runtime receiver refines the
        // static owner, so a subtype that declares the member serves a call
        // made through the supertype (`EnumEntries.indexOf` → `EnumEntriesList`).
        val subtypes = HashMap<String, MutableList<ShimClass>>()
        for (c in index.values) for (s in supertypesOf(index, c)) subtypes.getOrPut(s) { ArrayList() } += c
        ...
        for (r in fixtureRefs) {
            ...
            when (r.kind) {
                in MEMBER_KINDS -> {
                    val reason = resolveMember(index, owner, r, contract, subtypes)
    ...
    private fun resolveMember(
        index: Map<String, ShimClass>,
        owner: ShimClass,
        r: Ref,
        contract: ApiContract.Contract,
        subtypes: Map<String, List<ShimClass>>,
    ): String? {
        ...
        if (subtypes[owner.name].orEmpty().any { c -> (if (isField) c.fields else c.methods).any { it.name == r.name && it.desc == r.desc } }) return null
        // The walk left the shim: a java/… supertype (class or interface) whose
        // member the generated contract serves is answered by the JVM's
        // runtime-class dispatch (`EnumEntries.size()` → `List.size()`,
        // `Pair.hashCode()` → Object's).
        val javaSupers = supertypesOf(index, owner).filter { it.startsWith("java/") }
        if (javaSupers.any { contract.servesMember(it, r.name, r.desc) }) return null
