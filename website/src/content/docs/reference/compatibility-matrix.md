...
|---|---|---|
| `Activity` | Full | Lifecycle (`onCreate`/`onStart`/`onResume`/`onPause`/`onStop`/`onRestart`/`onDestroy`), `onKeyDown`/`onKeyUp` (the fallback after the focused view's `OnKeyListener`, with Android's BACK tracking), `onBackPressed`, `startActivity`, `startActivityForResult` + `onActivityResult`, `setResult`, `getIntent`, `finish`. No `dispatchKeyEvent`, `onKeyLongPress` or key repeat. |
...
| `KeyEvent` | Partial | D-pad / button codes, plus `KEYCODE_BACK` and `KEYCODE_HOME`; `getAction`, `getKeyCode`, `startTracking`/`isTracking`. Both special keys are handled by the framework before an app sees them: BACK falls through the soft keyboard, a showing dialog, the focused View, then `Activity.onKeyDown`/`onKeyUp`, whose defaults run `onBackPressed`; HOME goes straight to the launcher and cannot be intercepted, as on Android. No `getRepeatCount`, `isLongPress`, meta state or characters. Which physical buttons exist is `board.toml`'s business. |
