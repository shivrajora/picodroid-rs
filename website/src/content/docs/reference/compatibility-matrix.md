...
| `HashMap.entrySet()` / `Map.Entry` | Partial | `entrySet()`, `keySet()` and `values()` views answer `iterator()` and `size()` only; entries answer `getKey()`/`getValue()` (no `setValue`). Iteration order is the map's internal order. |
