...
| Installed apps | 8 (RP2350 boards, `max_installed_apps`), 1 (RP2040) | a ninth package is refused with `NoRoom`; a reinstall of an installed package replaces it |
...
- **Installed apps.** The package directory holds `max_installed_apps` packages (8 on the RP2350 boards); a package is identified by its manifest `package=`, and installing one that is already there upgrades it in place of the old copy rather than taking a second entry. RP2040 boards keep one app, which every install replaces.
