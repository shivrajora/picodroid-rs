...
| `LV_FONT_MONTSERRAT_*` | `14` only | Montserrat 14 is the theme face and `LV_FONT_DEFAULT`. The larger faces `TextView.setTextSize` can snap to are not LVGL's stock ones: a board's `text_sizes` key (MCU or board toml, `14;20;28;64` on the RP2350s, `14` on the RP2040) compiles ASCII-only subsets generated by `scripts/gen-fonts.sh` into `crates/pd-lvgl-sys/lvgl/fonts/`, at about half the flash of a stock face, and `pd_fonts.c` assembles the table the runtime reads. |
