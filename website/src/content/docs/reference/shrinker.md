...
Nine release maps are committed today:
...
| `sdk/shrink-maps/v0.5.0.toml` | Adds the **soft-keyboard polish** surface — `picodroid.view.OnEditorActionListener`, `picodroid.widget.EditorInfo`, plus internal anchor classes for the slide-up animation. Every v0.4.0 mapping copied verbatim. |
...
1. **Host pre-flight** in [tools/pdb/src/install.rs](https://github.com/shivrajora/picodroid-rs/blob/main/tools/pdb/src/install.rs):
   ...
   rejects.
2. **Device-side check** in [src/packagemanager/install.rs](https://github.com/shivrajora/picodroid-rs/blob/main/src/packagemanager/install.rs):
