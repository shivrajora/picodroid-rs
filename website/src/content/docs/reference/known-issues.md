...
- **`volatile` is ignored and the interpreter emits no memory barriers.** Correct only while a single core interprets Java, which is the invariant every JVM-adjacent task is pinned to today but which nothing enforces. Tracked as X1/THR-04 in `docs/parity-audit.md`.
