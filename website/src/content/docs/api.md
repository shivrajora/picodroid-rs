...
| [Services & DI](/api/services/) (Preview) | `picodroid.app`, `picodroid.content`, `javax.inject`, `picodroid.di` | `Service` / `IBinder` / `Notification`, `bindService` / `startService`, `ServiceConnection`, compile-time DI (`@Inject` / `@Singleton`, `Provider<T>` / `Lazy<T>`, automatic injection of `Application` / `Activity` / `Service`), manual DI components (`ApplicationComponent`, `ActivitySingletonComponent`) |
