...
- **Same-core install flow** — PDB and JVM tasks are both pinned to core 0 (an RP2350 cross-core SRAM visibility bug retired the original cross-core park design); during install the JVM blocks on a FreeRTOS notification, and each flash erase/program window disables interrupts inside `with_xip_disabled!` ([flash.rs](https://github.com/shivrajora/picodroid-rs/blob/main/platforms/rp/src/hal/rp/flash.rs)). ESP32-S3 has cache-suspension APIs (`esp_flash_suspend_cache`) that obviate this pattern.
