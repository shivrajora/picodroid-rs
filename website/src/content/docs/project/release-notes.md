...
**Every app has its own storage, a settings app, and apps that uninstall apps (multi-app M3)**
...
- Flash, release images against the `-Os` baselines below: `testbench_rp2350` +36,064 B (911,827 → 947,891: the settings PAPK, the new classes and natives, the quota); `testbench_rp2040` +10,116 B (783,547 → 793,663: the sandbox, the `Context` file API, `File.list`, `StatFs`, `Build`). Map v0.22.0 is due: until it is cut, a `--shrink` image carries the six new class names, the one expected `pre-commit --full` failure.
