...
- Every Kotlin example — `hellokt`, `langsuite_kt`, `langsuite_kt_stdlib`, `injectdemo_kt`, `gcstress_kt` — now passes on hardware (`pico_enviro_mon_w`, both shrink modes), and `picoenvmon_kt` completed a multi-hour device soak with a flat native footprint (286 KB at 20 min, 288 KB at 7.5 h) and no JVM-heap growth.
