...
- `TextView.setTextSize(float)`, `setTextSize(int unit, float)`, `getTextSize()` and
  ...
  (`COMPLEX_UNIT_*`, `applyDimension`) and `picodroid.util.DisplayMetrics`
  (`Resources.getDisplayMetrics()`), and `android:textSize` in a layout — a dimension, so
  `@dimen/` references work. `Button` inherits it and a content-sized button grows with its
  face. The one divergence: the faces are bitmaps, so a size snaps to the nearest compiled face
  (ties go up) and `getLineHeight()` reports the face in use. One density: `px`, `dp` and `sp`
  are one pixel, `PT` / `IN` / `MM` are nominal at 160 dpi.
- Boards choose their ladder with a `text_sizes` key (MCU or board toml): `14;20;28;64` on the
  RP2350 boards, `14` on the RP2040 testbench, where every size snaps to 14. Sizes above 14 are
  ASCII-only Montserrat faces generated by `scripts/gen-fonts.sh` into
  `crates/pd-lvgl-sys/lvgl/fonts/` (half the flash of LVGL's stock faces); `pd_fonts.c`
  assembles the table the runtime reads, so nothing is compiled a board did not list.
...
- **Dialogs** cover the whole display and are centred on it (the scrim was a 240 px square; on the 320 px testbench a tap beside it reached the rows behind the dialog), without a frame around the buttons or a scrollbar on the card. The harness taps the settings dialog's Uninstall button at a position derived from the board's width (`lib.sh::settings_dialog_ok`).
- **`Build.VERSION.RELEASE`** is the firmware release (`0.22.0`), generated into `build_info.rs`; it read `0.0.0` on every unshrunk image, the framework-map sentinel, and Settings > About showed it.
- **Launcher and settings** columns scroll (a `LinearLayout` does not, as on Android; a swipe on the testbench launched the row under the finger, and the Enviro's focus moved onto rows that never scrolled into view); rows hold one line; the Apps list is sorted like the launcher's, both case-insensitively; the About and Storage rows are focusable, so a keypad walks and scrolls them.
- **Simulator**: `apps install` reads the PAPK outside the simulated heap cap (an 800 KB app was "out of memory"), the package verbs reach an `Application` that has no Activity (blinky could not be uninstalled), every install stops the running app and brings it back — a launcher that comes back lists the new app — and a refused install says why in words. A device that has nothing left to run no longer reports the exited app as `running` in `pdb list`.
- Flash: the RP2350 release image +1,468 B against the v0.22.0 baseline (the settings and launcher PAPKs, the focus border, the modal group); the RP2040 −664 B.

**Every app has its own storage, a settings app, and apps that uninstall apps (multi-app M3; map v0.22.0, package 0.22.0)**

- **The storage sandbox.** Every path an app names — `new File("/x")`, a `SharedPreferences` file, `getFilesDir()` — now lives under the app's own `/data/<package>` on the one LittleFS volume, on every board: `..` is refused, an app path is at most 185 bytes, and no path can reach another app's files. Existing examples run unchanged; files a dev board held at the volume root are no longer visible to apps (they are not deleted). `Context` gains `getDataDir`, `getFilesDir` (`/files`), `openFileOutput` (`MODE_PRIVATE`, `MODE_APPEND`), `openFileInput`, `fileList` and `deleteFile`; `File` gains `list()` and `listFiles()`. Write-side failures throw `java.io.IOException` instead of a VM fault: `FileOutputStream.write`, `File.createNewFile` and the `openFile*` methods declare it. Data goes with its package: `pdb uninstall` and `PackageInstaller.uninstall` remove `/data/<package>`, and a multi-app device sweeps orphaned directories at boot. See [storage](/api/storage/).
- **The reserve and the cap** (multi-app boards). `fs_system_reserve_kb` (64) keeps the tail of the volume for the system; `app_data_cap_kb` (a quarter of `fs_kb`, 128 KB on the RP2350 boards; `0` lifts it) bounds one app's directory, in LittleFS's currency — 4 KB blocks per file, an 8 KB pair per directory. `picodroid.os.StatFs` reports the volume and what the app may still write; `picodroid.app.usage.StorageStatsManager` (`getSystemService(STORAGE_STATS_SERVICE)`) the bytes per package; `picodroid.os.Build` the board, MCU and release. `examples/quotademo` and `examples/filesdemo` prove the rules; both are bench rows.
- **The settings app.** `system-apps/settings` (`picodroid.settings`) sits beside the launcher in every multi-app firmware: About (board, MCU, release, volume, heap), Apps (one row per installed app; a tap opens a dialog and Uninstall removes the app and its data through `PackageInstaller`), Storage (app and data bytes per package). On a touch board the header row goes up one level and the root's header is Home. `PackageManager.getPackageInstaller().uninstall(name)` is the API behind it, synchronous, refusing a system app or the caller with `IllegalArgumentException`.
- **Simulator.** Its host image now has the board's volume size (`fs_kb`) and an image of another size is started afresh; `sim.sh --system-apps` loads the settings app too, and a new `settings` lane in `sim-run.sh` walks its screens.
- Flash, release images against the `-Os` baselines below: `testbench_rp2350` +36,064 B (911,827 → 947,891: the settings PAPK, the new classes and natives, the quota); `testbench_rp2040` +10,116 B (783,547 → 793,663: the sandbox, the `Context` file API, `File.list`, `StatFs`, `Build`). Map v0.22.0 folds the six classes (`Build`, `Build.VERSION`, `StatFs`, `StorageStatsManager`, `StorageStats`, `PackageInstaller`) and their 40 member names in; the member floor stays at v0.17.0, so PAPKs shrunk with v0.17.0 through v0.21.0 still install. `Build.VERSION.RELEASE` reads `0.22.0`.
...
- **What boots.** `flash.sh --boot app|launcher|<package>`, then a board's `boot_package`, then the app `flash.sh --app` baked in, then the launcher, then the lowest run. A freshly flashed app always wins over an older copy a `pdb install` left behind. In the simulator `PICODROID_BOOT` does the same at run time, and `sim.sh --system-apps` loads the launcher; without it the simulator runs one app and exits, as before.
- **Starting another app.** `Intent.setPackage(name)`, or `getPackageManager().getLaunchIntentForPackage(name)`, with `startActivity`. One app runs at a time: the current app is torn down through its normal lifecycle and the other one starts; when it finishes, the launcher comes back. A package that is not installed throws `ActivityNotFoundException`. There is no task stack, and extras do not cross over.
- **`PackageManager` queries.** `getInstalledPackages`, `getPackageInfo`, `getLaunchIntentForPackage`, `getApplicationLabel`, `getApplicationIcon`, with `PackageInfo`, `ApplicationInfo` (`FLAG_SYSTEM`) and `PackageManager.NameNotFoundException`; `Context.getPackageManager` and `Context.getPackageName`. `BitmapDrawable` carries another app's icon into `ImageView.setImageDrawable` or `View.setBackground`. On single-app boards only `hasSystemFeature` stays; the rest leaves that board's framework. A focusable view now scrolls into view when it takes focus.
- **`pdb list`** adds the system rows and a `running: <package>` line; the protocol stays `picodroid/2.2`. New bench rows `launch` and `launch-soak` (twenty launch-and-return cycles with no heap drift) and a nightly simulator lane cover the switch. See the [launcher guide](/guides/launcher/).
- Flash: `testbench_rp2350` +27,600 B (the launcher PAPK, the new classes and natives), `testbench_rp2040` +7,988 B (the larger `PackageManager`, `Intent` and `Context` classes and the boot selection). Map v0.21.0 folds the five classes and their 31 member names in; the member floor stays at v0.17.0, so PAPKs shrunk with v0.17.0 through v0.20.0 still install.
...
**Smaller PAPKs: `--shrink-app` renames the app's own classes and members**

- **`--shrink-app` (on `build-apk.sh`, `build.sh`, `flash.sh` and `sim.sh`; requires `--shrink`) extends the release map to the app itself**, ProGuard-style: the app's classes move under a third prefix, `c/`, its private method and field names shrink to one or two characters, and the descriptors naming them follow. Each PAPK build cuts its own map (`class-shrink cut-app`, Gradle task `cutAppShrinkMap`) by continuing the release map's allocator, and `build-apk.sh` writes it next to the PAPK as `build/apks/<app>.shrink-map.toml` — that file is the PAPK's retrace key (`./scripts/retrace.sh build/apks/<app>.shrink-map.toml < log`). `picoenvmon`'s stripped PAPK: 49,929 → 40,632 B (−18.6 %); the rp2350 release image that embeds it, 917,393 → 908,096 B. The firmware is untouched and `framework-map-version` is unchanged, so a `--shrink-app` PAPK installs on any `--shrink` firmware of the same release.
- **What it means for an app:** `Class.getName()` on your own class returns `c.A` and log lines spell `c/A.qZ` (`examples/classlit` accepts either spelling); string literals are never rewritten; the manifest keeps naming `myapp/MyApp` and `papk-pack` maps the entry class for you; `main` and the generated `injectMembers` stay verbatim, and a `<X>_MembersInjector` follows its component's new name so `@Inject` keeps resolving; the `kotlin/**` shim rides through untouched. Classes in the default package, or in a package literally named `a`, `b` or `c`, are rejected at build time. Details on the [shrinker page](/reference/shrinker/#app-shrinking---shrink-app).
...
- **`--shrink` is faster than no-shrink again, by design.** Uniform four-byte class names had turned every class-table scan in the JVM (`find_method`, the superclass walk, `new`) into a `bcmp` call per candidate — 57 M compare calls per sim `benchmark` run against 3.2 M unshrunk, and a 16 % slower `TOTAL` (`string_operations` +73 %). Every registered class now carries a name hash and every scan goes through one `find_class`; the builtin dispatch table carries the same hash. Compare calls: 372 k shrunk / 310 k unshrunk. On `testbench_rp2350` the old shrink image was +19.5 % overall (+138 % string operations, +69 % allocation, 20 runs each, σ ≤ 5 ms); the fixed shrink image is −18 % against it and the fastest of the four measured, and in the sim `--shrink` and no-shrink are at parity overall with the lookup-heavy sections 3–5 % faster shrunk. Costs ~0.9 KB of flash. Caveat for anyone reading device numbers: a 120-byte shift of the interpreter loop in XIP flash moves pure-arithmetic sections by up to 40 %, so compare only the sections a change touches (`docs/designs/unconditional-shrink-2026-09.md` §8.3).
...
- **`picodroid_core::porting` is the porting checklist.** It re-exports every seam item a family implements — the 20 HAL, kernel and hook traits and the 21 registration and logging macros — and its module doc is the seven-part checklist; two tests keep it and the [porting guide](/reference/porting-guide/) complete, with the counts pinned. The guide is rewritten around it: the real tree, one table for the HAL traits, sections for the kernel (`scheduler_running`, the `FsWorker` pin), the hooks, the filesystem, the debug bridge and installer, the simulator, the supervisor loop, logging, `build.rs` and the guards a port inherits.
- **Less to port.** The simulator now boots from one place in core (`register_sim_platform!` takes the family's GC roots, boot-budget model and app entry), and the boot-budget engine, the PAPK flash slot, the GPIO event ring, the scripted-touch override, the Java-array bus wrappers and the USB identity all moved out of `platforms/rp` into `picodroid-core` and `pdb-protocol`. `platforms/rp/src` shrank 8,774 → 7,952 lines and the rp2040 release image by 1,316 B. `ARCHITECTURE.md` gains the boundary rules a port must respect, each backed by a test.
- **`[background_pool] priority` must be 15**, the JVM's own tier, and the build fails otherwise. The porting guide's old row documented a default of 5 and a 1–10 range; neither was ever accepted.
