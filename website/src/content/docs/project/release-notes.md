...
- Flash: `testbench_rp2350` +36,064 B (1,004,755 → 1,040,819) (the settings PAPK, the new classes and natives, the quota); `testbench_rp2040` +10,132 B (802,679 → 812,811) (the sandbox, the `Context` file API, `File.list`, `StatFs`, `Build`), after the rp2040 `-Os` change above. Map v0.22.0 is due: until it is cut, a `--shrink` image carries the six new class names, the one expected `pre-commit --full` failure.
