...
| `bundledemo` | `bundledemo.BundleDemoApp` | `Bundle` conformance, `Intent.putExtras()` / `getExtras()`, and saved instance state across `Activity.recreate()` — `onSaveInstanceState` → `onCreate(Bundle)` → `onRestoreInstanceState` |
| `reclaimdemo` | `reclaimdemo.ReclaimDemoApp` | Reclaimed Activities: run with `PICODROID_DONT_KEEP_ACTIVITIES=1` and every covered Activity is destroyed with its state saved, then re-created from the Bundle when uncovered — including a `startActivityForResult` caller that gets its result as a new instance |
...
| `claudeusage` | `claudeusage.ui.MainActivity` | A desk display for Claude usage limits on the 320x240, four-button `pico_display2_w`: 5-hour session, weekly and per-model caps, burn rate and token history, fetched as JSON over plain HTTP from a small bridge on the LAN (`bridge/claude_usage_bridge.py`, with a `--demo` mode for the simulator). One Activity declared in the manifest, its header and footer inflated from `res/layout` and its colours and strings from `res/values`; four screens as pages, built a few views per UI tick and faded in; large numerals from `res/drawable` sprites; the two limits as `CircularProgressIndicator` ring gauges, the per-model meters as bars from plain views, there being no text-size API or styled linear ProgressBar; a started-and-bound `Service` with a poll thread and a ticker thread posting to the main executor; every failure (PC off, bridge down, login expired, stale data, a window resetting while offline) has its own presentation |
