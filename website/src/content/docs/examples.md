...
| `claudeusage` | `claudeusage.ClaudeUsageApp` | A desk display for Claude usage limits on the 320x240, four-button `pico_display2_w`: 5-hour session, weekly and per-model caps, burn rate and token history, fetched as JSON over plain HTTP from a small bridge on the LAN (`bridge/claude_usage_bridge.py`, with a `--demo` mode for the simulator). Four screens as pages in one Activity, built a few views per UI tick and faded in; large numerals from image sprites and bars from plain views, there being no text-size API or styled ProgressBar; one poll thread and one ticker thread posting to the main executor; every failure (PC off, bridge down, login expired, stale data, a window resetting while offline) has its own presentation |
