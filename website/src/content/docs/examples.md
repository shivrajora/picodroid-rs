...
| `injectdemo` | `injectdemo.InjectDemoApp` | Compile-time DI end to end: `@Inject` constructor / field / method injection, `@Singleton` identity across an `Application`, two Activities and a `Service` (all injected automatically before `onCreate`), an unscoped class per injection site, a leaf Activity that only inherits its `@Inject` members, `Foo_Factory.get()` from plain code, `Provider<T>` / `Lazy<T>` wrappers, and coexistence with a hand-written `ApplicationComponent` |
