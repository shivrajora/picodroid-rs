...
impl FileAllocation {
    /// `Err(NoMemory)` when the cache cannot be reserved: on an embedded
    /// heap `vec![0u8; cache_size]` (4 KB per open) aborted the firmware
    /// instead — picodroid change, see the note at the top of Cargo.toml.
    pub(crate) fn new(cache_size: u32) -> Result<Self, Error> {
        let mut cache = Vec::new();
        cache
            .try_reserve_exact(cache_size as usize)
            .map_err(|_| Error::NoMemory)?;
        cache.resize(cache_size as usize, 0u8);
