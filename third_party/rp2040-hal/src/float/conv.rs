...
intrinsics! {
    ...
    // picodroid: the ROM's float_to_int family rounds toward -infinity, but
    // the `__aeabi_*2iz`/`*2lz` contract (and Rust's `as`, C's cast, Java's
    // f2i) truncates toward zero. Negative inputs go through the ROM negated
    // — floor(-f) = trunc(f) for f < 0 — and the one value the negation
    // cannot represent, the type's minimum, is answered directly. NaN and
    // out-of-range inputs keep the ROM's clamping answers.
    #[aeabi = __aeabi_f2iz]
    extern "C" fn __fixsfsi(f: f32) -> i32 {
        if f <= -2147483648.0 {
            i32::MIN
        } else if f < 0.0 {
            rom_data::float_funcs::float_to_int(-f).wrapping_neg()
        } else {
    ...
    extern "C" fn __fixsfdi(f: f32) -> i64 {
        if f <= -9223372036854775808.0 {
            ...
            rom_data::float_funcs::float_to_int64(-f).wrapping_neg()
        } else {
    ...
    extern "C" fn __fixdfsi(f: f64) -> i32 {
        if f <= -2147483648.0 {
            ...
            rom_data::double_funcs::double_to_int(-f).wrapping_neg()
        } else {
    ...
    extern "C" fn __fixdfdi(f: f64) -> i64 {
        if f <= -9223372036854775808.0 {
            ...
            rom_data::double_funcs::double_to_int64(-f).wrapping_neg()
        } else {
