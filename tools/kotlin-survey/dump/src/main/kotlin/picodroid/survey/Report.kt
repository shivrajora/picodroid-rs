...
data class Tuple(
    ...
    val loadBearing: Boolean
        get() = kinds.any { it !in NON_LOAD_BEARING_KINDS }

    val pkg: String
        get() = owner.substringBeforeLast('/', "")
}
...
fun tuples(refs: List<Ref>): List<Tuple> =
    refs
        .groupBy { Triple(it.owner, it.name, it.desc) }
        .map { (k, rs) ->
            Tuple(
                k.first,
                k.second,
                k.third,
                rs.mapTo(sortedSetOf()) { it.kind },
                rs.size,
                rs.mapTo(sortedSetOf()) { it.sourceFile },
                rs.mapTo(sortedSetOf()) { it.fromClass },
            )
        }
        .sortedWith(compareBy({ it.owner }, { it.name }, { it.desc }))

val REFS_HEADER =
    listOf("kind", "owner", "name", "desc", "from_class", "from_member", "source_file", "detail")
...
fun tuplesTsv(ts: List<Tuple>): String =
    tsv(
        listOf(
            "owner",
            "name",
        ...
        ts.map {
            listOf(
                ...
                it.name,
                it.desc,
                it.kinds.joinToString("|"),
                it.count,
                it.loadBearing,
                it.sourceFiles.joinToString("|"),
                it.fromClasses.joinToString("|"),
            )
        },
    )

/** Attribute names → census column, in column order. */
val ATTR_COLUMNS: List<Pair<String, List<String>>> =
    listOf(
        "code_bytes" to listOf("Code"),
        "rva_bytes" to listOf("RuntimeVisibleAnnotations"),
        "ria_bytes" to listOf("RuntimeInvisibleAnnotations"),
        "rpa_bytes" to
            listOf("RuntimeVisibleParameterAnnotations", "RuntimeInvisibleParameterAnnotations"),
...
/**
 * Attribute columns the strip removes (everything pico-jvm skips by length except what we keep for
 * line numbers).
 */
val STRIPPED_ATTR_COLUMNS: List<String> =
    listOf(
        "rva_bytes",
        "ria_bytes",
        "rpa_bytes",
        "rta_bytes",
        "annotation_default_bytes",
        "signature_bytes",
        "stackmap_bytes",
        "lvt_bytes",
        "lvtt_bytes",
        "innerclasses_bytes",
        "enclosingmethod_bytes",
        "nest_bytes",
        "methodparameters_bytes",
        "sde_bytes",
    )
...
val CENSUS_HEADER: List<String> =
    listOf(
        ...
        ATTR_COLUMNS.map { it.first } +
        listOf("other_attr_bytes") +
        listOf(
            "cp_tag15",
            "cp_tag16",
            "cp_tag17",
...
fun censusRow(c: Census): List<Any?> =
    listOf(
        c.className,
        c.sourceFile,
        c.major,
        ...
        c.bytes,
        c.cpCount,
        "0x" + c.access.toString(16),
        c.superName,
        c.interfaces.joinToString("|"),
        c.fields,
        c.methods,
        c.synchronizedMethods,
        c.defaultMethods,
        c.bridgeMethods,
        c.syntheticMethods,
        c.defaultArgBridges,
        ...
        ATTR_COLUMNS.map { attrColumn(c, it.first) } +
        listOf(otherAttrBytes(c)) +
        listOf(
            c.cpTag(15),
            c.cpTag(16),
            c.cpTag(17),
...
fun cpClassesTsv(cs: List<Census>): String =
    tsv(
...
fun indyTsv(rows: List<IndyRow>): String =
    tsv(
        INDY_HEADER,
        rows.map {
            listOf(
...
fun summaryMarkdown(label: String, results: List<ClassResult>, external: Regex): String =
    buildString {
        ...
        censuses
            .filter { it.cpTag(17) + it.cpTag(19) + it.cpTag(20) > 0 }
            .forEach {
                flags +=
                    "CP tag 17/19/20 in `${it.className}` (17=${it.cpTag(17)}, 19=${it.cpTag(19)}, 20=${it.cpTag(20)}) — pico-jvm rejects the class at registration"
            }
        allRefs
            .filter { it.kind == "ldc_condy" }
            .forEach {
                flags += "`ldc` of a dynamic constant in `${it.fromClass}.${it.fromMember}`"
            }
        indyRows
            .filter { !it.isMetafactory }
            .forEach {
                flags +=
                    "non-LambdaMetafactory bootstrap `${it.bsmOwner}.${it.bsmName}` in `${it.fromClass}.${it.fromMember}` — ops_invoke.rs would misread arguments[1]"
            }
        indyRows
            .filter { it.isMetafactory && it.bsmName == "altMetafactory" }
            .forEach {
                flags +=
                    "`altMetafactory` in `${it.fromClass}.${it.fromMember}` (extra args: ${it.extraArgs})"
            }
        indyRows
            .filter { it.implRefKind.startsWith("8:") }
            .forEach {
                flags +=
                    "`REF_newInvokeSpecial` impl handle `${it.implOwner}.<init>${it.implDesc}` in `${it.fromClass}.${it.fromMember}` — would call <init> without `new`"
            }
        censuses
            .filter { it.className.endsWith("\$DefaultImpls") }
            .forEach { flags += "`${it.className}` exists — -Xjvm-default=all did not apply" }
        censuses
            .filter { it.major > 52 }
            .forEach { flags += "`${it.className}` has class version ${it.major} (> 52)" }
        ...
        table(
            ...
            byPkg.map { (p, xs) ->
                listOf(
                    p,
                    xs.map { it.owner }.toSet().size,
                    xs.size,
                    xs.count { it.loadBearing },
                    xs.sumOf { it.count },
                )
            },
        )
        table(
            ...
            byOwner.map { (o, xs) ->
                listOf(
                    ...
                    xs.size,
                    xs.count { it.loadBearing },
                    xs.flatMap { it.kinds }.toSortedSet().joinToString(" "),
                    xs.flatMap { it.sourceFiles }.toSortedSet().joinToString(" "),
                )
            },
        )
        ...
        append(
            "`load` = load-bearing (needs a class file / dispatch arm in pico-jvm); rows with only "
        )
        append(NON_LOAD_BEARING_KINDS.sorted().joinToString("/") { "`$it`" })
            .append(" kinds do not.\n\n")
        byPkg.forEach { (p, xs) ->
            append("### `$p`\n\n")
            table(
        ...
        table(
            listOf("bootstrap", "sites"),
            histogram(indyRows.map { "${it.bsmOwner}.${it.bsmName}" }),
        )
        ...
        table(
            listOf("impl owner package", "sites"),
            histogram(indyRows.map { it.implOwner.substringBeforeLast('/', it.implOwner) }),
        )
        table(
            listOf(
                "from",
            ...
            indyRows.map {
                listOf(
                    "${it.fromClass}.${it.fromMember}",
                    it.samInterface,
                    "${it.indyName}${it.indyDesc}",
                    "${it.bsmOwner}.${it.bsmName}",
                    "${it.implOwner}.${it.implName}${it.implDesc}",
                    it.implRefKind,
                    it.instantiatedDesc,
                    it.extraArgs,
                )
        ...
        censuses.forEach { c ->
            c.attrBytes.forEach { (k, v) -> attrTotals.merge(k, v, Int::plus) }
        }
        table(
            listOf("attribute", "bytes (len+6, all occurrences)"),
            attrTotals.map { (k, v) -> listOf(k, v) },
        )
        table(
            listOf("metric", "total"),
            listOf(
                ...
                listOf(
                    "classes named *\$WhenMappings",
                    censuses.count { it.className.endsWith("\$WhenMappings") },
                ),
                listOf(
                    "classes named *\$Companion",
                    censuses.count { it.className.endsWith("\$Companion") },
                ),
                listOf(
                    "classes named *\$DefaultImpls",
                    censuses.count { it.className.endsWith("\$DefaultImpls") },
                ),
                listOf(
                    "anonymous/local classes (*\$<digit>)",
                    censuses.count { Regex("\\$\\d+$").containsMatchIn(it.className) },
                ),
            ),
        )
        table(
            ...
            censuses.map {
                listOf(
                    ...
                    it.cpCount,
                    it.methods,
                    it.synchronizedMethods,
                    it.bridgeMethods,
                    it.syntheticMethods,
                    it.defaultArgBridges,
                    attrColumn(it, "rva_bytes"),
                    attrColumn(it, "stackmap_bytes"),
                    attrColumn(it, "lvt_bytes"),
                    attrColumn(it, "innerclasses_bytes"),
                    attrColumn(it, "signature_bytes"),
                    if (it.hasMetadata) "yes" else "",
                )
            },
        )
    }
...
fun stripStatsTsv(rows: List<Pair<String, StripStats>>): String {
    ...
    val total =
        StripStats(
            rows.sumOf { it.second.bytesBefore },
            rows.sumOf { it.second.bytesAfter },
            rows.sumOf { it.second.cpBefore },
            rows.sumOf { it.second.cpAfter },
        )
    return tsv(
        listOf(
            "class",
        ...
        body + listOf(stripRow("TOTAL", total)),
    )
}
...
private fun stripRow(name: String, s: StripStats): List<Any?> {
    ...
    return listOf(
        ...
        s.cpBefore,
        s.cpAfter,
        s.cpBefore - s.cpAfter,
    )
}

fun stripSummaryMarkdown(label: String, rows: List<Triple<String, Census, StripStats>>): String =
    buildString {
        ...
        table(
            ...
            listOf(
                listOf(
                    ...
                    after,
                    before - after,
                    "%.1f".format(if (before == 0) 0.0 else (before - after) * 100.0 / before),
                ),
                listOf(
                    ...
                    "%.1f"
                        .format(
                            if (cpBefore == 0) 0.0 else (cpBefore - cpAfter) * 100.0 / cpBefore
                        ),
                ),
            ),
        )
        ...
        append(
            "Attribute bytes (header included) present before the strip, by column; the remainder of the saving is constant-pool entries only those attributes referenced (e.g. `@Metadata` protobuf strings).\n\n"
        )
        val perCol = STRIPPED_ATTR_COLUMNS.map { col ->
            col to rows.sumOf { attrColumn(it.second, col) }
        }
        val attrSum = perCol.sumOf { it.second }
        table(
            listOf("column", "bytes"),
            perCol.map { (c, b) -> listOf(c, b) } +
                listOf(
                    listOf("(attributes total)", attrSum),
                    listOf("(constant pool + other)", (before - after) - attrSum),
                ),
        )
        append("## Per class\n\n")
        table(
            ...
            rows.map { (n, _, s) ->
                stripRow(n, s).let { listOf(it[0], it[1], it[2], it[3], it[4], it[5], it[6]) }
            },
        )
    }
