...
use types::JvmError;
#[cfg(all(not(test), feature = "threaddemo"))]
use types::Value;
...
// ── Per-feature class loading helpers ────────────────────────────────────────

#[cfg(all(not(test), feature = "threaddemo"))]
pub fn load_threaddemo_classes(jvm: &mut Jvm) -> Result<(), JvmError> {
...
pub fn run_jvm() -> ! {
    ...
    {
        load_threaddemo_classes(&mut jvm).unwrap();
