...
impl Jvm {
    ...
    pub fn invoke_static(&mut self, class_name: &str, method_name: &str) -> Result<(), JvmError> {
        ...
        interpreter::execute(&self.classes, &mut self.strings, ci, mi, &[])?;
        Ok(())
    }
}
...
pub fn run_jvm() -> ! {
    ...
    jvm.load_class(APPS_HELLOWORLD_CLASS).unwrap();
    jvm.load_class(PICODROID_UTIL_LOG_CLASS).unwrap();

    jvm.invoke_static("apps/HelloWorld", "main").unwrap();

    loop {
        freertos_rust::CurrentTask::delay(freertos_rust::Duration::ms(10_000));
    }
}
