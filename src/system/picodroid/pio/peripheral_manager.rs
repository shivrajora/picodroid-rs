...
pub fn open_i2c(
    ...
    objects
        .set_field(obj_idx, 0, Value::Int(i2c_id as i32))
        .ok_or(JvmError::StackOverflow)?;
    objects
        .set_field(obj_idx, 1, Value::Int(100_000))
        .ok_or(JvmError::StackOverflow)?;

    // Initialize hardware: GPIO function select + I2C enable at 100 kHz
    #[cfg(not(test))]
    super::i2c::init(i2c_id);
...
mod tests {
    ...
    fn open_i2c_i2c0_sets_id_0_and_default_speed() {
        ...
        assert_eq!(objects.get_field(idx, 0), Some(Value::Int(0)));
        assert_eq!(objects.get_field(idx, 1), Some(Value::Int(100_000)));
    }
