...
if [[ "$MODE" == "check" ]]; then
  ...
  if ! find "$REPO_ROOT/examples" "$REPO_ROOT/sdk" "$REPO_ROOT/kotlin-shim" "$REPO_ROOT/inject" -type d -name build -prune -o -name '*.java' -print0 | xargs -0 java -jar "$JAR_PATH" --dry-run --set-exit-if-changed; then
    echo ""
  ...
  find "$REPO_ROOT/examples" "$REPO_ROOT/sdk" "$REPO_ROOT/kotlin-shim" "$REPO_ROOT/inject" -type d -name build -prune -o -name '*.java' -print0 | xargs -0 java -jar "$JAR_PATH" --replace
  echo "==> Done."
else
