...
| 17 | Progress bars are nested `FrameLayout`s with `GradientDrawable`s. | SDK-forced (no styled `ProgressBar`, no `Canvas`) | **partly closed (2026-09-23)**: the Limits gauges are the SDK's `CircularProgressIndicator` (`lv_arc`); the Models page's bars stay nested `FrameLayout`s until a per-instance-coloured linear `ProgressBar` exists (gaps G3). |
...
5. A styled linear `ProgressBar` or a minimal `Canvas` (items 17, 18; gaps G3, G4). The ring form landed as `CircularProgressIndicator` (gaps G2).
