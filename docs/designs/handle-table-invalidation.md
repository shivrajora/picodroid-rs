...
- CI cfg-matrix hole in staging: with `handle-table-32` default-off, pre-commit clippy (RP2040+RP2350) and the embedded build never compile the unified table's 32-bit/no_std arm between commit 2 and the commit-3 default flip; no_std violations (std leakage in the sanitizer path, provenance casts, defmt format types) would surface only at flip time.
