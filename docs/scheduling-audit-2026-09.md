...
| WP7 tick timebase (F16) | landed 2026-09-13 as `719d43e7`, **reverted the same day**: every tick-loop app (animdemo, alarmdemo) stalled on the RP2350 W board. **2026-09-15: that slot was stalling painting apps on its own** — a silent 5 s SPI completion wait (`f74c108e`, `docs/qa-2026-09-13-followups.md` item 1) — and on the fixed tree WP7's `animdemo` meets both of its row's criteria there. A candidate to re-land: `docs/scheduling-audit-handover-2026-09.md` §2 WP7 |
...
| WP0, WP7 (retry) | open — WP7 re-measured clean 2026-09-15 and is ready to re-land; see the plan below and the handover doc |
