...
## Appendix C — the 32-bit simulator lane (2026-07-28)

The audit's environment note recorded a missing oracle: *"no gcc-multilib
(32-bit C differential oracle not available — device-as-oracle used instead)."*
That gap is now closed from the other direction. The simulator cross-compiles
to `armv7-unknown-linux-gnueabihf` and runs under qemu-user, so a development
machine can execute it with the device's pointer width and instruction set:

```bash
./scripts/sim.sh --arch arm32 --app helloworld   # one app
./scripts/test.sh --arch arm32                   # unit tests, incl. handle-table-32
./scripts/sim-run.sh --arch both --no-email      # full matrix, tagged app[mode/arm32]
```

It is opt-in everywhere and absent from pre-commit, which must not require an
emulator. What it changes for the divergences catalogued above:

- **OBJ-02 / OBJ-03 are gone on this lane.** `Frame`'s four `usize` fields and
  `StringTable`'s pointer vector are the residual host inflation §6 called
  "not currently worth" deleting. At 32 bits they are simply device-sized:
  helloworld peaks at **88 KB on arm32 vs 95 KB natively**, and the arm32
  figure is the one that matches hardware.
- **HAL-05 is reachable on a development machine.** At 32 bits the default
  build compiles the *device's* handle path — the raw-pointer cast that
  dangles after widget delete — instead of the generational table a 64-bit
  host always selects. The handle sanitizer is a no-op there, which is not a
  regression but the point: that is what the devices run.
- **The `handle-table-32` staging config now executes.** Its 32-bit arm was
  previously compiled by the pre-commit legs and never run anywhere;
  `scripts/test.sh --arch arm32` runs the table's tests (ABA slot reuse,
  generation wrap, forged high bits) at 32 bits and against the board's
  `HANDLE_SLOTS` rather than the 64-bit path's fixed 1024. `sim-run.sh`'s
  arm32 lane adds a `callbacktest[no-shrink/arm32-ht32]` behavioral run. This
  is coverage the default-flip decision (P1-9) did not previously have.
- **papk-format's 32-bit arithmetic is testable.** `docs/designs/papk-format-crate.md`
  flags offset arithmetic that cannot overflow when `usize` is 64-bit. Under
  this target `usize == u32`, and `profile.test` keeps overflow checks on.

Verified when the lane landed: graphicsbench under `parity-metrics` +
`parity-fbhash` produces **all 1000 band CRCs identical** between the native
and arm32 simulators, with `insns=147899 allocs=4831 gcs=18 bands=1000
fbytes=9648348` equal on both — the same counters and CRCs Appendix B recorded
as equal between the native sim and the RP2350, so the chain now closes
transitively to hardware.

What it does **not** buy, so §6's limits stand: instruction timing, XIP cache
behavior, and preemption granularity are unmodelled as ever; and thumbv6-M's
missing atomic RMW is an ISA property of the RP2040, not of pointer width —
armv7 has LDREX/STREX, so the "no `fetch_add` on RP2040-reachable paths" rule
still needs its thumbv6m compile gate.
