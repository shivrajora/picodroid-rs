...
| MEM-08 | Sim main-stack conditions differ from device: firmware BSS is 509.6 KB of 520 KB RAM leaving ~10 KB Cortex-M main-stack headroom (measured at build, `arm-none-eabi-size`), thinner than the ~28 KB the FreeRTOSConfig comment claims. 2026-09-17: the size(1) figure omitted the executable `.data`; the linked stack is `_stack_start - _stack_end`, which the build now prints and the linker script floors at 8 KB (`__main_stack_floor`) after the touch kit's 2,048 B stack overflowed in the boot-time LittleFS sweep | — | Device-side robustness observation surfaced by the audit (stale comment); not a sim divergence per se | S3 | V | doc fix; link-time floor 2026-09-17 |
...
| THR-06 | `Thread.setPriority` is **advisory** on every target: all Java-interpreting tasks (UI task, `Thread.start` children, the background pool) run at the one JVM tier (FreeRTOS 15). The shared heap is lock-free on "a running JVM task keeps the core until it blocks", which needs time slicing off *and* equal priorities — until 2026-08-30 children ran at Android+10 (11–20) and the pool at 5, either of which let one Java task preempt another mid-heap-mutation or mid-LVGL call. Android treats the value as a scheduling hint; here the hint is stored and reported but never applied (`task_priority.rs`) | IPB | A CPU-bound `MAX_PRIORITY` thread gets no more CPU than a `MIN_PRIORITY` one; a busy loop at any priority starves equal-priority siblings until it blocks (slicing is off by design) | S3 | V | by design — the alternative is a global interpreter lock. **2026-09-16:** that lock now exists for a different reason: the JVM run lock (`d1a09765`, `designs/jvm-run-lock-2026-09.md`) lets one task interpret at a time on both targets, because FreeRTOS rotated equal-priority Java tasks mid-instruction whenever a higher-priority task woke and blocked. Priorities stay advisory; a busy Java loop still starves its siblings, which now wait Blocked on the mutex rather than Ready |

### Time / tick (TIM)

| ID | Divergence | Class | Symptom | Sev | Conf | Fix |
|---|---|---|---|---|---|---|
| TIM-01 | LVGL tick = fixed 16 ms step on both; backing = FreeRTOS software timer (device) vs `std::thread` + `Instant` pacing (sim) (`executors/tick_source.rs:25-128`). Animation *phase* is deterministic and parity-identical; wall-clock pacing differs. **2026-09-16:** both halves are dated. The sim has run the same FreeRTOS timer since M7, and since WP7 (`d8563ae3` + `9d090232`, 2026-09-15) the step is `tick_source::step_ms()` — one period while the loop keeps up, the measured lateness when it falls behind — on both targets, so animation phase now follows wall-clock time and a slow board no longer runs its animations slow. `Display.update()` keeps its fixed one-period step by contract. Same code both sides; what still differs is how late a device tick drains | IS/IPB | Frame-count-deterministic behavior is a genuine parity asset; only real-time pacing diverges | S3 | V | — |
...
| TIM-04 | `SimDelay::delay_ns` is a no-op; device busy-waits on the cycle counter. **2026-09-12:** the device side is the bigger problem — `RpDelay` is `cortex_m::asm::delay` and the panel and touch drivers run it from the JVM task after the scheduler starts (350–540 ms at init, 120 ms per screen wake on the UI task); replaced 2026-09-12 (`docs/scheduling-audit-2026-09.md` F2 / WP2): `RpDelay` now sleeps on the kernel for anything ≥ 1 ms and spins only a sub-tick remainder | IPB | Driver timing paths run instantaneous in sim (mostly device-only code anyway) | S3 | V | register-only |
...
| ID | Divergence | Class | Symptom | Sev | Conf | Fix |
|---|---|---|---|---|---|---|
| DSP-01 | Render path **shared end-to-end**: one `lv_conf.h` (`LV_COLOR_FORMAT_RGB565_SWAPPED` since 2026-09-16, previously RGB565 + `LV_COLOR_16_SWAP 1`; SW renderer), same partial-band draw buffer, same `flush_cb`; sim converts the finished BE-RGB565 band to ARGB8888 into a host `FRAMEBUF`, minifb 2× upscale strictly after (`picodroid-core/src/hal/sim/display.rs`) | IS | The framebuffer bytes the harness should hash are device-identical by construction — the graphics tier's foundation | — | V | G1 exploits this |
...
- **class-shrink**: Utf8 lengths are written `as u16` (unreachable while renames only
  shorten) and any string literal containing `/` is treated as a class name and rewritten —
