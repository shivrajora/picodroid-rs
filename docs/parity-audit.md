...
| TIM-04 | `SimDelay::delay_ns` is a no-op; device busy-waits on the cycle counter. **2026-09-12:** the device side is the bigger problem — `RpDelay` is `cortex_m::asm::delay` and the panel and touch drivers run it from the JVM task after the scheduler starts (350–540 ms at init, 120 ms per screen wake on the UI task); `docs/scheduling-audit-2026-09.md` F2 / WP2 replaces it with an RTOS-backed delay | IPB | Driver timing paths run instantaneous in sim (mostly device-only code anyway) | S3 | V | register-only |
