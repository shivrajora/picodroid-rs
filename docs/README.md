...
| [designs/claudeusage-gaps-roadmap-2026-09.md](designs/claudeusage-gaps-roadmap-2026-09.md) | open | Platform defects (**D4, top priority: every tick of a page swap overruns the slow-handler budget on the RP2350**; sim connect timeouts firing late on an unreachable host, LEFT_RIGHT gradients, `View.close()` leak) and UI gaps (font sizes, styled ProgressBar, charts, asset alpha, backlight PWM, BuildConfig, key dispatch; the arc gauge, G2, landed 2026-09-23) found building the `claudeusage` dashboard. |
