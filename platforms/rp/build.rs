...
fn main() {
    ...
    if is_embedded {
        ...
        if mcu_family == "rp" {
            ...
            if board.props.get("network_type").map(String::as_str) == Some("cyw43") {
                let heap_kb = mcu_heap_kb(&mcu, &mcu_toml_path);
    ...
    emit_heap_config(out, &board_cfg_full);
    ...
    emit_handle_table_config(out, &board_cfg_full);
...
fn mcu_heap_kb(mcu: &std::collections::HashMap<String, String>, mcu_toml_path: &str) -> u32 {
    mcu.get("heap_kb")
...
/// (`sim_allocator.rs`), keeping it single-sourced with the FreeRTOS C
...
fn emit_heap_config(out: &Path, board: &Option<config::BoardConfig>) {
    let heap_bytes = match board {
        Some(bc) => {
            ...
            let mcu = config::parse_toml(&mcu_toml_path);
            println!("cargo:rerun-if-changed={mcu_toml_path}");
            mcu_heap_kb(&mcu, &mcu_toml_path) as usize * 1024
        }
        None => 416 * 1024,
    };
    let body = format!(
    ...
    std::fs::write(out.join("heap_config.rs"), body).unwrap();
}
...
fn emit_jvm_config(out: &std::path::Path, board: &Option<config::BoardConfig>) {
    ...
    let gc_alloc_threshold =
        read_jvm_u32(jvm, "gc_alloc_threshold", &jvm_defaults::GC_ALLOC_THRESHOLD);
    let slot_chunk_shift = read_jvm_u32(jvm, "slot_chunk_shift", &jvm_defaults::SLOT_CHUNK_SHIFT);
    let inline_array_data =
        read_jvm_u32(jvm, "inline_array_data", &jvm_defaults::INLINE_ARRAY_DATA);
    ...
    // 1b. Boot-time heap pre-reservation constants for app.rs (PEM-3).
    let code = format!(
        "// Generated by build.rs from board.toml [jvm] — do not edit.\n\n\
         pub const PRERESERVE_OBJ_CHUNKS: usize = {prereserve_obj_chunks};\n\
         pub const PRERESERVE_ARR_CHUNKS: usize = {prereserve_arr_chunks};\n\
         pub const PRERESERVE_STR_CHUNKS: usize = {prereserve_str_chunks};\n\
         pub const PRERESERVE_FIELDS_VALUES: usize = {prereserve_fields_values};\n\
         pub const PRERESERVE_ARENA_VALUES: usize = {prereserve_arena_values};\n"
    );
    let path = out.join("jvm_prereserve_config.rs");
    let mut f = File::create(&path).expect("create jvm_prereserve_config.rs");
    f.write_all(code.as_bytes())
        .expect("write jvm_prereserve_config.rs");
...
/// non-integer or out-of-range → panic with the field name + value. Default
/// and range come from `build_support/jvm_defaults.rs`, which `jvm/build.rs`
/// also imports — drift between the two crates is impossible.
fn read_jvm_u32(
...
/// `handle_slots` key in board.toml: the LVGL widget handle table's slot
/// count on 32-bit builds (concurrently-live widgets, not cumulative).
/// Absent key → 256. Must be a power of two (the table decodes the slot
/// index by mask) in 32..=4096. Consumed by
/// `src/system/picodroid/graphics/lvgl/handle_table.rs`.
fn emit_handle_table_config(out: &std::path::Path, board: &Option<config::BoardConfig>) {
    const DEFAULT_HANDLE_SLOTS: usize = 256;
    let slots = board
        .as_ref()
        .and_then(|b| b.props.get("handle_slots"))
    ...
    let path = out.join("handle_table_config.rs");
    let mut f = File::create(&path).expect("create handle_table_config.rs");
    f.write_all(code.as_bytes())
        .expect("write handle_table_config.rs");
}
