...
fn main() {
    ...
    if is_embedded {
        ...
        if mcu_family == "rp" {
            ...
            // The module's PSRAM, when the descriptor declares one: the two
            // inputs hal/rp/psram.rs needs beyond the layout's origin and
            // length — the pad that carries XIP_CS1n, and the system clock
            // the QMI window's timing is derived from.
            if layout.psram_len > 0 {
                let cs_pin: u8 = mcu
                    .get("psram_cs_pin")
                    .unwrap_or_else(|| {
                        panic!("MCU toml declares psram_kb but no 'psram_cs_pin': {mcu_toml_path}")
                    })
                    .trim()
                    .parse()
                    .unwrap_or_else(|e| panic!("MCU toml 'psram_cs_pin' ({mcu_toml_path}): {e}"));
                let clock_hz: u64 = mcu
                    .get("clock_hz")
                    .unwrap_or_else(|| panic!("MCU toml missing 'clock_hz': {mcu_toml_path}"))
                    .trim()
                    .parse()
                    .unwrap_or_else(|e| panic!("MCU toml 'clock_hz' ({mcu_toml_path}): {e}"));
                std::fs::write(
                    out.join("psram_config.rs"),
                    format!(
                        "// Generated by build.rs from {mcu_toml_path} — do not edit\n\
                         pub const PSRAM_CS_PIN: u8 = {cs_pin};\n\
                         pub const PSRAM_CLOCK_HZ: u64 = {clock_hz};\n"
                    ),
                )
                .unwrap_or_else(|e| panic!("write psram_config.rs: {e}"));
            }
            freertos::build(
                out,
                &mcu,
