...
// Where the regions sit is generated by build.rs from the MCU toml and the
// board's overrides (build_support/flash_layout.rs): FLASH_ORIGIN,
// PROGRAM_LEN, FS_OFFSET/FS_LEN, PAPK_REGION_OFFSET/PAPK_REGION_LEN and
// MAX_INSTALLED_APPS — the same numbers the linker script's MEMORY block was
// rendered from, so the two cannot drift.
include!(concat!(env!("OUT_DIR"), "/flash_layout.rs"));

/// Mapped address of the app region (`PAPK_FLASH`).
pub const PAPK_FLASH_XIP_BASE: usize = FLASH_ORIGIN + PAPK_REGION_OFFSET as usize;

// The boot-meta layout belongs to `papk_format::flash_image`, the run
// arithmetic on top of it to `picodroid_core::install::PapkRegion`, and the
// directory of what the region holds to `picodroid_core::packages`. Only
// where the region *sits* on this family's flash, and how big it is, is
// ours to say.

/// The app region, read in place: XIP maps it, so the package directory
/// walks it without copying.
pub fn region_base() -> *const u8 {
    PAPK_FLASH_XIP_BASE as *const u8
}

/// XIP base address for both supported chips.
pub const XIP_BASE: usize = FLASH_ORIGIN;
