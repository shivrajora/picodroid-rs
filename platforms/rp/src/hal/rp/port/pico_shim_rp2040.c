...
void multicore_launch_core1(void (*entry)(void)) {
     ...
     * at the same table, so the FIFO handlers each core installs afterwards
     * (port.c xPortStartSchedulerOnCore) are live on BOTH cores.  Launching
     * with whatever core 0's VTOR held at the time left one core on the
     * flash table, where its SIO FIFO slot is the cortex-m-rt DefaultHandler
     * infinite loop: the first cross-core yield IPI wedged that core at the
     * lowest IRQ priority, blocking its tick and PendSV forever. */
    ensure_ram_vt();
