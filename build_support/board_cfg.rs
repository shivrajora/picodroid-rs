...
/// Emit `OUT_DIR/build_info.rs`: the board and MCU names `picodroid.os.Build`
/// reports (`BOARD`, `MCU`); `host` for a boardless build.
pub fn emit_build_info(out: &Path, board: &Option<ResolvedBoard>) {
    let (board_name, mcu_name) = match board {
        Some(b) => (b.name.clone(), config::resolve_active_mcu(&b.cfg.props)),
        None => ("host".to_string(), "host".to_string()),
    };
    let code = format!(
