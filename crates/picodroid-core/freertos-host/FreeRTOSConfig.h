 ...
 *     runs tasks on pthread stacks sized by pthread, not by us. The tick
 *     hook is on only under `sched-diag` (PICODROID_SCHED_DIAG, the block
 *     at the end of this file), where it feeds the scheduling monitor
 *     exactly as it does on a device; the idle hook never is (see the hook
 *     section for why the monitor's printer is a host thread here).
 *
...
/* Hook functions — see note 3 in the header comment. The idle hook stays
 * off even under sched-diag: the port deletes the idle task with
 * pthread_cancel at scheduler end, and a cancel that unwinds into a Rust
 * frame aborts the process (hal/sim/rtos_freertos.rs); the monitor prints
 * from a host thread instead. */
