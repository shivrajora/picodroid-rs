...
use crate::hal::event_ring::GpioEventRing;
...
/// Synthetic button-edge queue: the device's ring, the device's size. On
/// hardware these edges come from the GPIO IRQ; the host sim has no GPIO
/// peripheral, so the input front-ends in `hal::sim::display` (keyboard +
/// control channel) call [`inject`] instead. A `Mutex` around the ring
/// because the control-channel front-end injects from a background thread —
/// the ring's own contract is one producer at a time.
///
/// Bounded on purpose: a stalled simulator now drops and warns exactly as a
/// stalled device does, where the old unbounded queue hid the overflow
/// (docs/parity-audit.md TCH-01).
static EVENTS: Mutex<GpioEventRing<64>> = Mutex::new(GpioEventRing::new());
...
pub fn inject(pin: u8, rising: bool) {
    // `GpioEvent::t_us` is real host time captured at inject time, mirroring
    // the ISR capture on hardware: the debounce compares enqueue-time deltas,
    // so a synthetic edge with a stale stamp would land inside the window and
    // be eaten.
    let t_us = (super::system_clock::elapsed_realtime_nanos() / 1_000) as u32;
    EVENTS.lock().unwrap().enqueue(pin, rising, t_us);
}

pub fn drain_gpio_event() -> Option<GpioEvent> {
    EVENTS.lock().unwrap().drain()
}

pub fn has_pending_event() -> bool {
    EVENTS.lock().unwrap().has_pending()
}
