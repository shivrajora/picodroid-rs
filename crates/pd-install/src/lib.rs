...
//!
//! It lives in a crate of its own, outside any chip family, because none of
//! it is about silicon: it is a wire protocol, a compatibility gate, a placement policy,
...
//! What is *firmware*-specific — which runs are installed and where the next
//! one goes — arrives through [`PackageDirectory`], and the framework-map
//! version the firmware was built against is an argument. So the crate
//! depends on the container format and the wire protocol and nothing else.
//!
...
//! whole or invisible. The tests in [`orchestrator`] pin the sequence;
//! picodroid-core's `packages` tests pin it again against the real directory.

#![cfg_attr(not(any(test, feature = "mem-region")), no_std)]

extern crate alloc;

mod directory;
#[cfg(any(test, feature = "mem-region"))]
pub mod mem_region;
mod orchestrator;
pub mod region;
pub mod transport;

pub use directory::{run_sectors, PackageDirectory, Plan, PlanError};
