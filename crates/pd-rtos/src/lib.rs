...
/// The JVM run lock. It lives beside the wrappers because they release it:
/// see the module docs for why that is not a hook a caller registers.
pub mod run_lock;
...
pub enum TaskKind {
    ...
    ///
    /// Only the simulator creates this through the seam (`picodroid_core::sim_boot`).
    ...
    /// (`picodroid_core::hal::touch_sampler`). Exists only on boards whose
    /// controller has a bus of its own.
    Touch,
    /// The filesystem's serial worker
    /// (`picodroid_core::executors::serial_worker`).
    ///
    ...
    FsWorker,
    /// The debug-bridge listener (`picodroid_core::pdb::run_pdb_task`).
    ///
    /// Like [`TaskKind::Jvm`], only the simulator creates this through the
    /// seam (`picodroid_core::sim_boot`); a device's boot code creates its bridge
...
pub unsafe trait Rtos {
    ...
    /// — `picodroid_core::fs::with_fs` is the caller. [`Rtos::task_current`] cannot
...
///
/// See `picodroid_core::hal`'s macros for the symbol-name contract these share.
#[macro_export]
macro_rules! set_rtos {
    ($t:ty) => {
        const _: () = {
            use $crate::{RawMutex, RawQueue, RawSem, RawTask, TaskSpec, Timeout};
